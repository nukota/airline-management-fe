...
const TicketRow: React.FC<{ ticket: TicketType; index: number }> = ({
  ticket,
  index,
}) => {
  return (
    <tr key={ticket.ticketId}>
      <th key={ticket.ticketId}>
        <label>
          <span>{index}</span>
        </label>
      </th>
      ...
      <td>
        <div className="tooltip" data-tip={ticket.seatFlight.bookingId}>
          <span className="font-semibold">
            {ticket.seatFlight.bookingId?.slice(0, 8).concat("...")}
          </span>
        </div>
      </td>
      <td>
        <div
          className="tooltip"
          data-tip={`${ticket.fullName} - ${ticket.phoneNumber} ${ticket.email} `}
        >
          <span
            className="font-semibold"
            onClick={(e: any) => {
              console.log(e);
              navigator.clipboard.writeText(ticket.passengerId);
            }}
          >
            {ticket.passengerId.slice(0, 8).concat("...")}{" "}
          </span>
        </div>
      </td>
      <td>
      ...
      <td>
        <div className="tooltip" data-tip={ticket.seatFlight.class}>
          <span className="font-semibold text-base">
            {ticket.seatFlight.seatId}
          </span>
        </div>
      </td>
      ...
      <td>
        SellAt: <span className="font-semibold">{ticket.sellAt}</span>
        <br />
        UpdateAt: <span className="text-sm">{ticket.updateAt}</span>
      </td>
      <td>
      ...
      <td className="flex flex-col mt-3 justify-center">
        <Dropdown key={ticket.ticketId} className="flex justify-center">
          <DropdownTrigger>
